...
/* HAMED PANEL v1.0.5 — Silent Edition */
...
const ALL_PERMISSIONS = ["users","settings","advanced","managers","apikeys","logs","stats","subscriptions","nodes","backup","groups","cron","webhooks","regions"];
...
async function getEncryptionKey(masterKey) {
  ...
  const key = await crypto.subtle.deriveKey({ name:"PBKDF2", salt: enc.encode("hamed-panel-enc-v1"), iterations: 50000, hash:"SHA-256" }, baseKey, { name:"AES-GCM", length:256 }, false, ["encrypt","decrypt"]);
...
const SYSTEM_DEFAULTS = {
  ...
  crisisPresets: [
    { id:"cut", title:"⚠️ قطعی سراسری", text:"⚠️ توجه: در حال حاضر اینترنت سراسری دچار اختلال است. تیم ما در حال رفع مشکل است." },
    { id:"slow", title:"🐢 کندی سرعت", text:"🐢 ممکن است در ساعات اوج مصرف، سرعت اینترنت شما کاهش یابد." },
    { id:"update", title:"🔄 بروزرسانی سرور", text:"🔄 سرورها در حال بروزرسانی هستند. لطفاً کانفیگ خود را مجدداً دریافت کنید." },
  ],
  crisisHistory: [],
  // v1.0.5 new fields
...
async function deployWorkerToCloudflare(accountId, apiToken, workerName, code) {
  let cb = [];
  try { const r = await fetch(`https://api.cloudflare.com/client/v4/accounts/${accountId}/workers/scripts/${encodeURIComponent(workerName)}/settings`, { headers:{ Authorization:`Bearer ${apiToken}` } }); const j = await r.json(); if (j.success && j.result?.bindings) cb = j.result.bindings; } catch (e) {}
...
async function triggerWebhook(env, ctx, event, payload) {
  ...
  for (const wh of hooks) {
    ...
    const p = fetch(wh.url, { method:"POST", headers:{ "Content-Type":"application/json", "X-Hamed-Event":event, "X-Hamed-Signature":"sha256=" + sig, "User-Agent":"HamedPanel/" + CURRENT_VERSION }, body, signal: AbortSignal.timeout(10000) }).catch(() => {});
...
async function sendCrisisBroadcast(env, message, presetId) {
  if (!sysConfig.tgToken) return { success:false, error:"Telegram not configured" };
  ...
  for (const id of recipients) {
    try { const r = await fetch(`https://api.telegram.org/bot${sysConfig.tgToken}/sendMessage`, { method:"POST", headers:{ "Content-Type":"application/json" }, body: JSON.stringify({ chat_id:id, text:message, parse_mode:"HTML" }) }); const j = await r.json(); if (j.ok) sent++; else failed++; } catch (e) { failed++; }
  }
...
export default {
  async fetch(request, env, ctx) {
    try {
      ...
      // v1.0.5 FORCE migration sync → sub
      ...
      // v1.0.5 normalize /sync/* to /sub/* internally (no 301 — POST body preserved!)
  ...
  async scheduled(event, env, ctx) {
    try {
      ...
      if (sysConfig.autoUpdate && sysConfig.cfAccountId && sysConfig.cfApiToken && sysConfig.cfWorkerName) {
        ...
        if (repo) {
          let rv = null;
          try { const r = await fetch(`https://raw.githubusercontent.com/${repo}/main/version`); if (r.ok) rv = (await r.text()).trim(); } catch (e) {}
          if (rv && cmpVersions(CURRENT_VERSION, rv) < 0) {
            try { let r = await fetch(`https://raw.githubusercontent.com/${repo}/main/_worker.encode.js`); if (!r.ok) r = await fetch(`https://raw.githubusercontent.com/${repo}/main/_worker.js`); if (r.ok) { const code = await r.text(); const dr = await deployWorkerToCloudflare(sysConfig.cfAccountId, sysConfig.cfApiToken, sysConfig.cfWorkerName, code); const dres = await dr.json(); if (dres.success) await logActivity(env, "Auto-Update Success", `v${rv}`); } } catch (e) {}
          }
        }
      }
    } catch (e) {}
...
async function fetchCloudflareUsage(accountId, apiToken) {
  ...
  try {
    ...
    const r = await fetch("https://api.cloudflare.com/client/v4/graphql", { method:"POST", headers:{ Authorization:`Bearer ${apiToken}`, "Content-Type":"application/json" }, body: JSON.stringify({ query, variables:{ a:accountId, s:start } }) });
...
async function handleUsersApi(request, env, ctx) {
  try {
    ...
    if (method === "POST" && !userId) {
      ...
      const { name, trafficLimit, expiryDays, notes, maxConfigs, proxyIp, cleanIp, userMode, userPorts, userNodes, nat64, connLimit, userPanelUrl, groupId, autoReset, isp, bandwidthKbps } = body;
      ...
      await cachedD1Put(env, "sys_config", JSON.stringify(sysConfig));
      ctx?.waitUntil(logActivity(env, "User Created", `${name} by ${perm.ctx.username}`).catch(() => {}));
      ctx?.waitUntil(triggerWebhook(env, ctx, "user.created", { userId:newId, name, groupId:newUser.groupId }).catch(() => {}));
  ...
  try {
    ...
    if (method === "POST") {
      ...
      if (body.action === "create") {
        ...
        if (!username || !password) return new Response(JSON.stringify({ ok:false, success:false, error:"username & password required" }), { status:400 });
        if (String(username).length < 3) return new Response(JSON.stringify({ ok:false, success:false, error:"min 3 chars" }), { status:400 });
        if (String(password).length < 4) return new Response(JSON.stringify({ ok:false, success:false, error:"min 4 chars" }), { status:400 });
        if (sysConfig.managers.some(m => m.username.toLowerCase() === String(username).toLowerCase())) return new Response(JSON.stringify({ ok:false, success:false, error:"username exists" }), { status:400 });
      ...
      if (body.action === "update") {
        ...
        if (isActive !== undefined) { if (m.isRoot && !isActive) return new Response(JSON.stringify({ ok:false, success:false, error:"Cannot deactivate root" }), { status:400 }); m.isActive = !!isActive; }
        await cachedD1Put(env, "sys_config", JSON.stringify(sysConfig));
        return new Response(JSON.stringify({ ok:true, success:true }));
      }
      if (body.action === "delete") {
        ...
        if (sysConfig.managers[idx].isRoot) return new Response(JSON.stringify({ ok:false, success:false, error:"Cannot delete root" }), { status:400 });
...
async function handleStatsApi(request, env) {
  try {
    ...
    return new Response(JSON.stringify({ ok:true, success:true, data:{ users:{ total, active, paused, expired, autoDisabled:autoOff }, traffic:{ totalRequests: totalReqs, totalGB:(totalReqs/6000).toFixed(2), dailyRequests: dailyReqs, dailyGB:(dailyReqs/6000).toFixed(2) }, system:{ uptimeSeconds: Math.floor((Date.now()-isolateStartTime)/1000), activeConnections, version: CURRENT_VERSION, isPaused: sysConfig.isPaused || false, topUsers } }, stats:{ users:{ total, active, paused, expired, autoDisabled:autoOff }, traffic:{ totalRequests: totalReqs, totalGB:(totalReqs/6000).toFixed(2), dailyRequests: dailyReqs, dailyGB:(dailyReqs/6000).toFixed(2) }, system:{ uptimeSeconds: Math.floor((Date.now()-isolateStartTime)/1000), activeConnections, version: CURRENT_VERSION, isPaused: sysConfig.isPaused || false, topUsers } } }), { headers:{ "Content-Type":"application/json" } });
  } catch (e) { return new Response(JSON.stringify({ ok:false, success:false, error:e.message }), { status:500 }); }
}
...
async function handleWebhooksApi(request, env, ctx) {
  try {
    ...
    if (method === "POST") {
      ...
      if (body.action === "test") { const w = (sysConfig.webhooks || []).find(x => x.id === body.id); if (!w) return new Response(JSON.stringify({ ok:false, success:false }), { status:404 }); const payload = JSON.stringify({ event:"test", timestamp:Date.now(), version:CURRENT_VERSION, data:{ message:"Test webhook" } }); const sig = await hmacSign(w.secret, payload); try { const r = await fetch(w.url, { method:"POST", headers:{ "Content-Type":"application/json", "X-Hamed-Event":"test", "X-Hamed-Signature":"sha256=" + sig }, body:payload, signal: AbortSignal.timeout(8000) }); return new Response(JSON.stringify({ ok:true, success:true, status: r.status })); } catch (e) { return new Response(JSON.stringify({ ok:false, success:false, error: e.message })); } }
    }
    return new Response(JSON.stringify({ ok:false, success:false }), { status:400 });
  } catch (e) { return new Response(JSON.stringify({ ok:false, success:false, error:e.message }), { status:500 }); }
}
...
async function handleUpdateApi(request, env, ctx) {
  try {
    ...
    if (data.action === "check") {
      ...
      let rv = null;
      try { const r = await fetch(`https://raw.githubusercontent.com/${repo}/main/version`); if (r.ok) { const t = (await r.text()).trim(); if (t && t.length <= 15) rv = t; } } catch (e) {}
    ...
    if (data.action === "deploy") {
      ...
      let code = data.code;
      if (!code) { try { let r = await fetch(`https://raw.githubusercontent.com/${repo}/main/_worker.encode.js`); if (!r.ok) r = await fetch(`https://raw.githubusercontent.com/${repo}/main/_worker.js`); if (r.ok) code = await r.text(); else throw new Error("HTTP " + r.status); } catch (e) { return new Response(JSON.stringify({ ok:false, success:false, error:e.message }), { status:502 }); } }
...
const WORKFLOW_TRIGGERS = [
  ...
  { id:"usage.dailyGB", label:"مصرف روزانه از X GB گذشت" },
...
async function fireWorkflows(env, ctx, trigger, triggerCtx) {
  try {
    ...
    for (const wf of wfs) {
      let ok = true;
      if (wf.conditions) {
        if (wf.conditions.minGB && (triggerCtx.gb || 0) < parseFloat(wf.conditions.minGB)) ok = false;
        if (wf.conditions.maxGB && (triggerCtx.gb || 0) > parseFloat(wf.conditions.maxGB)) ok = false;
...
async function handleSuggestions(request, env) {
  try {
    ...
    if (noIsp > 3) suggestions.push({ level:"info", icon:"🎛️", title:"تعیین ISP", desc:`${noIsp} کاربر اپراتور مشخصی ندارند.`, action:"tab:users" });
    if ((sysConfig.activeCleanRegions || []).length === 0) suggestions.push({ level:"warn", icon:"🌍", title:"فعال‌سازی مناطق IP", desc:"هیچ منطقه‌ای فعال نیست.", action:"tab:regions" });
    if (sysConfig.activeFragment === "off") suggestions.push({ level:"info", icon:"🎯", title:"فعال‌سازی Fragment", desc:"Fragment خاموش است.", action:"tab:advanced" });
    if (Date.now() - (sysConfig.lastBackup || 0) > 7 * 86400000) suggestions.push({ level:"warn", icon:"💾", title:"بکاپ بگیر", desc:"بیش از ۷ روز از آخرین بکاپ گذشته.", action:"tab:backup" });
    let over90 = 0;
    ...
    if (over90 > 0) suggestions.push({ level:"warn", icon:"🚨", title:`${over90} کاربر نزدیک محدودیت`, desc:"چند کاربر در آستانه اتمام ترافیک.", action:"tab:users" });
    if ((sysConfig.cronJobs || []).length === 0) suggestions.push({ level:"info", icon:"⏰", title:"ساخت Cron Job", desc:"هنوز تسک زمان‌بندی‌شده‌ای ندارید.", action:"tab:cron" });
    if ((sysConfig.webhooks || []).length === 0) suggestions.push({ level:"info", icon:"🔔", title:"افزودن Webhook", desc:"برای اتصال به Slack/Discord.", action:"tab:webhooks" });
    if ((sysConfig.workflows || []).length === 0) suggestions.push({ level:"info", icon:"🔄", title:"ساخت Workflow", desc:"اتوماسیون خودکار بسازید.", action:"tab:workflows" });
...
async function handlePredictive(request, env) {
  try {
    ...
    for (let i = 1; i <= days; i++) {
      ...
      predictions.push({ date:d.toISOString().split("T")[0], gb: parseFloat(projected.toFixed(3)) });
    }
...
async function handleAuth(request, hostName, ctx, env) {
  try {
    ...
    const now = Date.now();
    let rlData = rlRaw ? JSON.parse(rlRaw) : { count:0, first:now };
    if (!rlData.first || now - rlData.first > AUTH_WINDOW_MS) rlData = { count:0, first:now };
    if (rlData.count >= AUTH_MAX_ATTEMPTS * 3) return new Response(JSON.stringify({ ok:false, success:false, error:"Too many attempts", retryAfter: Math.ceil((AUTH_WINDOW_MS - (now - rlData.first))/1000) }), { status:429, headers:{ "Content-Type":"application/json" } });
    ...
    if (legacyKey) {
      ...
      if (!username || !password) return new Response(JSON.stringify({ ok:false, success:false, error:"username & password required" }), { status:400, headers:{ "Content-Type":"application/json" } });
      mgr = await verifyManagerCredentials(username, password);
      if (!mgr) {
        rlData.count++;
        await d1Put(env, rlKey, JSON.stringify(rlData));
    ...
    if (mgr && mgr.id) { const f = sysConfig.managers.find(m => m.id === mgr.id); if (f) { f.lastLogin = Date.now(); cachedD1Put(env, "sys_config", JSON.stringify(sysConfig)).catch(() => {}); } }
    const sess = await createSession(env, mgr, request);
    await d1Put(env, rlKey, "");
...
async function handleConfigSync(request, env, ctx) {
  try {
    ...
    if (nextConfig.tgToken && ctx) { const hook = `https://${new URL(request.url).hostname}/${encodeURI(nextConfig.apiRoute)}/tg`; ctx.waitUntil(fetch(`https://api.telegram.org/bot${nextConfig.tgToken}/setWebhook`, { method:"POST", headers:{ "Content-Type":"application/json" }, body: JSON.stringify({ url:hook }) }).catch(() => {})); }
  ...
  if (isRealBrowser && !isCustomUaAllowed) {
    if (isValidUser) {
      try {
        ...
        const fragHtml = frag ? `<div class="frg">🎯 Fragment فعال: <code>${frag}</code></div>` : "";
...
function getConfigName(type, profileName, port, hostName, ip, proxyIp = null, configIndex = 0, ipName = "", isDirect = false, regionInfo = null) {
  ...
  if (strategy.includes("{") && strategy.includes("}")) { const lookupIp = proxyIp || ip; const geo = getGeoInfo(lookupIp); const protoLab = type === "alpha" ? "VLESS" : "Trojan"; const now = new Date(); const dateStr = now.getFullYear() + "-" + String(now.getMonth()+1).padStart(2,"0") + "-" + String(now.getDate()).padStart(2,"0"); const workerName = sysConfig.cfWorkerName || sysConfig.name || hostName || ""; const flagToUse = isDirect ? "☁️" : (regionInfo && regionInfo.flag ? regionInfo.flag : geo.flag); const countryToUse = regionInfo ? regionInfo.name : geo.country; return strategy.replace(/{FLAG}/g, flagToUse).replace(/{COUNTRY}/g, countryToUse).replace(/{CITY}/g, geo.city).replace(/{ISP}/g, geo.isp).replace(/{PROTOCOL}/g, protoLab).replace(/{USER}/g, profileName).replace(/{PORT}/g, port).replace(/{PREFIX}/g, prefix).replace(/{IP}/g, ip||"").replace(/{IP_NAME}/g, ipName||"").replace(/{HOST}/g, hostName||"").replace(/{DATE}/g, dateStr).replace(/{INDEX}/g, String(configIndex)).replace(/{WORKER}/g, workerName); }
      ...
      ePorts.forEach(port => {
        ...
        ips.forEach(ip => {
          ...
          _pips.forEach(sel => {
            ...
            if (mode === "alpha" || mode === "both") { const cfgUuid = generateConfigUuid(p.id, configIndex); registerConfigEntry(cfgUuid, p.id, sel || ""); lines.push(`${getAlpha()}://${cfgUuid}@${ip}:${port}?${extBase}#${getConfigName("alpha", p.name, port, hName, ip, sel, configIndex, ipEntry.name||"", false, regionInfo)}`); }
            if (mode === "beta" || mode === "both") { const junk = Array.from({ length:11 }, () => "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"[Math.floor(Math.random()*62)]).join(""); const payload = { junk, protocol:"tr", mode:"proxyip", panelIPs:[], relayIdx:configIndex }; const pathStr = "/" + btoa(JSON.stringify(payload)); let tb = `security=${sec}&sni=${hName}&fp=${ispTemplate.agent || sysConfig.agent || "chrome"}&type=ws&host=${hName}&path=${encodeURIComponent(pathStr)}${ispFrag}`; if (sysConfig.enableOpt2) tb += `&pbk=enabled`; tb += `&allowInsecure=${allowInsecure ? "1" : "0"}`; lines.push(`${getBeta()}://${p.id}@${ip}:${port}?${tb}#${getConfigName("beta", p.name, port, hName, ip, sel, configIndex, ipEntry.name||"", false, regionInfo)}`); }
            configIndex++;
          });
        });
      });
      ...
      ePorts.forEach(port => {
        ...
        ips.forEach(ip => {
          ...
          _pips.forEach(sel => {
            const ipEntry = ipEntryMap[ip] || {};
            const regionInfo = ipEntry.regionId ? { id:ipEntry.regionId, name:ipEntry.regionName, flag:ipEntry.regionFlag } : null;
            if (mode === "alpha" || mode === "both") { const tag = uniqueName(getConfigName("alpha", p.name, port, hName, ip, sel, configIndex, ipEntry.name||"", false, regionInfo)); dynamicTags.push(tag); const junk = Array.from({ length:11 }, () => "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"[Math.floor(Math.random()*62)]).join(""); const pathStr = "/" + btoa(JSON.stringify({ junk, protocol:"vl", mode:"proxyip", panelIPs:[] })); const cfgUuid = generateConfigUuid(p.id, configIndex); registerConfigEntry(cfgUuid, p.id, sel || ""); proxiesArr.push({ name:tag, type:"vless", server:ip, port:parseInt(port), udp:true, uuid:cfgUuid, tls:sec, servername:hName, "client-fingerprint": ispTemplate.agent || "random", "skip-cert-verify":allowInsecure, network:"ws", "ws-opts":{ path:pathStr, headers:{ Host:hName } } }); }
            if (mode === "beta" || mode === "both") { const tag = uniqueName(getConfigName("beta", p.name, port, hName, ip, sel, configIndex, ipEntry.name||"", false, regionInfo)); dynamicTags.push(tag); const junk = Array.from({ length:11 }, () => "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"[Math.floor(Math.random()*62)]).join(""); const pathStr = "/" + btoa(JSON.stringify({ junk, protocol:"tr", mode:"proxyip", panelIPs:[], relayIdx:configIndex })); const cfgUuid = generateConfigUuid(p.id, configIndex); registerConfigEntry(cfgUuid, p.id, sel || ""); proxiesArr.push({ name:tag, type:"trojan", server:ip, port:parseInt(port), udp:true, password:p.id, tls:sec, sni:hName, "client-fingerprint": ispTemplate.agent || "random", "skip-cert-verify":allowInsecure, network:"ws", "ws-opts":{ path:pathStr, headers:{ Host:hName } } }); }
            configIndex++;
          });
        });
      });
    ...
    getProfileHostNames(hostName, p).forEach(hName => {
      ...
      ePorts.forEach(port => {
        ...
        ips.forEach(ip => {
          const _pips = pips.length > 0 ? pips : [null];
          _pips.forEach(sel => {
            if (mode === "alpha" || mode === "both") { const tag = uniqueName(getConfigName("alpha", p.name, port, hName, ip, sel, configIndex, "")); const cfgUuid = generateConfigUuid(p.id, configIndex); registerConfigEntry(cfgUuid, p.id, sel || ""); const path = "/" + btoa(JSON.stringify({ junk:"junk", protocol:"vl", mode:"proxyip", panelIPs:[], relayIdx:configIndex })); outbounds.push({ tag, protocol:"vless", settings:{ vnext:[{ address:ip, port:parseInt(port), users:[{ id:cfgUuid, encryption:"none" }] }] }, streamSettings:{ network:"ws", security:sec, tlsSettings: sec === "tls" ? { serverName:hName, allowInsecure } : undefined, wsSettings:{ path, headers:{ Host:hName } } } }); }
            if (mode === "beta" || mode === "both") { const tag = uniqueName(getConfigName("beta", p.name, port, hName, ip, sel, configIndex, "")); const path = "/" + btoa(JSON.stringify({ junk:"junk", protocol:"tr", mode:"proxyip", panelIPs:[], relayIdx:configIndex })); outbounds.push({ tag, protocol:"trojan", settings:{ servers:[{ address:ip, port:parseInt(port), password:p.id }] }, streamSettings:{ network:"ws", security:sec, tlsSettings: sec === "tls" ? { serverName:hName, allowInsecure } : undefined, wsSettings:{ path, headers:{ Host:hName } } } }); }
            configIndex++;
          });
        });
      });
    ...
    getProfileHostNames(hostName, p).forEach(hName => {
      ...
      ePorts.forEach(port => {
        ...
        ips.forEach(ip => {
          const _pips = pips.length > 0 ? pips : [null];
          _pips.forEach(sel => {
            if (mode === "alpha" || mode === "both") { const tag = uniqueName(getConfigName("alpha", p.name, port, hName, ip, sel, configIndex, "")); const path = "/" + btoa(JSON.stringify({ junk:"junk", protocol:"vl", mode:"proxyip", panelIPs:[] })); const cfgUuid = generateConfigUuid(p.id, configIndex); registerConfigEntry(cfgUuid, p.id, sel || ""); outbounds.push({ type:"vless", tag, server:ip, server_port:parseInt(port), uuid:cfgUuid, network:"tcp", tls:{ enabled:sec, server_name:hName, insecure:allowInsecure, utls:{ enabled:true, fingerprint:"randomized" } }, transport:{ type:"ws", path, headers:{ Host:hName } } }); }
            if (mode === "beta" || mode === "both") { const tag = uniqueName(getConfigName("beta", p.name, port, hName, ip, sel, configIndex, "")); const path = "/" + btoa(JSON.stringify({ junk:"junk", protocol:"tr", mode:"proxyip", panelIPs:[], relayIdx:configIndex })); outbounds.push({ type:"trojan", tag, server:ip, server_port:parseInt(port), password:p.id, network:"tcp", tls:{ enabled:sec, server_name:hName, insecure:allowInsecure, utls:{ enabled:true, fingerprint:"randomized" } }, transport:{ type:"ws", path, headers:{ Host:hName } } }); }
            configIndex++;
          });
        });
      });
...
async function handleTelegramWebhook(request, env, hostName, ctx) {
  try {
    ...
    if (!isAuthorized) { const chatId = update.callback_query?.message?.chat?.id || update.message?.chat?.id; if (chatId) await fetch(`${tgApi}/sendMessage`, { method:"POST", headers:{ "Content-Type":"application/json" }, body: JSON.stringify({ chat_id:chatId, text:"❌ دسترسی ندارید" }) }); return new Response("OK"); }
    let tgState = {};
    try { const s = await d1Get(env, "tg_bot_state"); if (s) tgState = JSON.parse(s); } catch (e) {}
    const sendMsg = async (chatId, text, kb = null, messageId = null) => {
      let res;
      if (messageId) { res = await fetch(`${tgApi}/editMessageText`, { method:"POST", headers:{ "Content-Type":"application/json" }, body: JSON.stringify({ chat_id:chatId, message_id:messageId, text, parse_mode:"Markdown", reply_markup:kb }) }); if (res.ok) return res; }
      return await fetch(`${tgApi}/sendMessage`, { method:"POST", headers:{ "Content-Type":"application/json" }, body: JSON.stringify({ chat_id:chatId, text, parse_mode:"Markdown", reply_markup:kb }) });
    };
    const mainMenu = () => {
      const users = sysConfig.users || [];
    ...
    if (update.callback_query) {
      const cb = update.callback_query, chatId = cb.message?.chat?.id, messageId = cb.message?.message_id;
      const m = mainMenu();
      await sendMsg(chatId, m.text, m.kb, messageId);
...
/* ═══════════════════════════════════════════════════════════════
   DASHBOARD HTML — v1.0.5 Silent Edition
   ═══════════════════════════════════════════════════════════════ */
const DASHBOARD_HTML = '<!DOCTYPE html><html lang="fa" dir="rtl"><head>' +
'<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover">' +
'<title>Hamed Panel · Silent</title><meta name="theme-color" content="#05070d">' +
...
'.mm{display:none;position:fixed;top:16px;right:16px;z-index:60;width:46px;height:46px;border-radius:14px;background:rgba(20,26,44,.95);border:1px solid var(--border-2);align-items:center;justify-content:center;cursor:pointer;color:var(--text);backdrop-filter:blur(12px)}' +
...
'.pt{font-size:26px;font-weight:900;background:var(--grad);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}' +
...
'.ct::before{content:"";width:4px;height:18px;border-radius:4px;background:var(--grad)}' +
...
'.fd label{font-size:12px;font-weight:700;color:var(--text-2)}' +
...
'.stt::before{content:"";width:4px;height:16px;background:var(--grad);border-radius:4px}' +
...
'.md{background:linear-gradient(180deg,#141a2c,#0c101c);border:1px solid var(--border-2);border-radius:24px;padding:26px;max-width:600px;width:100%;box-shadow:0 40px 100px -20px rgba(0,0,0,.7);max-height:90vh;overflow-y:auto;animation:pi .3s var(--ease)}' +
...
'.weather-icon{font-size:56px;filter:drop-shadow(0 0 20px rgba(139,92,246,.5))}' +
...
'<p style="text-align:center;color:var(--muted);font-size:13px;margin-bottom:26px">Silent Edition · v__CURRENT_VERSION__</p>' +
...
'<div>Silent Edition</div><div style="margin-top:4px;opacity:.6">v__CURRENT_VERSION__</div>' +
'</div></div></div>' +
'<div id="shell" class="shell">' +
'<div class="ov" id="ov" onclick="tg()"></div>' +
'<button class="mm" onclick="tg()"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 12h18M3 6h18M3 18h18"/></svg></button>' +
'<aside class="sb" id="sb">' +
'<div class="brand"><div id="brandLogo"></div>' +
'<div class="nm tg">__PANEL_NAME__</div><div class="tg2" id="whoami">Loading...</div></div>' +
'<nav style="display:flex;flex-direction:column;gap:2px" id="nav">' +
'<div class="nav-group">عمومی</div>' +
'<a class="nv on" data-tab="overview"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/></svg><span>داشبورد</span></a>' +
'<a class="nv" data-tab="weather"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="4"/><path d="M12 2v2M12 20v2M4.93 4.93l1.41 1.41M17.66 17.66l1.41 1.41M2 12h2M20 12h2M6.34 17.66l-1.41 1.41M19.07 4.93l-1.41 1.41"/></svg><span>وضعیت شبکه</span></a>' +
'<a class="nv" data-tab="users" data-perm="users"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/></svg><span>کاربران</span></a>' +
'<a class="nv" data-tab="groups" data-perm="groups"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/></svg><span>گروه‌ها</span></a>' +
'<a class="nv" data-tab="traffic" data-perm="stats"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 3v18h18"/><path d="M7 14l4-4 4 4 4-6"/></svg><span>ترافیک</span></a>' +
'<a class="nv" data-tab="anomalies" data-perm="stats"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg><span>هشدارها</span></a>' +
'<a class="nv" data-tab="predictive" data-perm="stats"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 3v18h18"/><path d="M18.7 8l-5.1 5.2-2.8-2.7L7 14.3"/></svg><span>پیش‌بینی</span></a>' +
'<a class="nv" data-tab="suggestions" data-perm="stats"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 18h6M10 22h4M12 2a7 7 0 0 0-4 12.7c.6.5 1 1.2 1 2.3h6c0-1.1.4-1.8 1-2.3A7 7 0 0 0 12 2z"/></svg><span>پیشنهادات</span></a>' +
'<div class="nav-group">مدیریت</div>' +
'<a class="nv" data-tab="managers" data-perm="managers"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2l3 7h7l-5.5 4.5 2 7.5L12 17l-6.5 4 2-7.5L2 9h7z"/></svg><span>مدیران</span></a>' +
'<a class="nv" data-tab="sessions" data-perm="managers"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="3"/><path d="M12 1v6M12 17v6M4.22 4.22l4.24 4.24M15.54 15.54l4.24 4.24M1 12h6M17 12h6"/></svg><span>نشست‌ها</span></a>' +
'<a class="nv" data-tab="nodes" data-perm="nodes"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="2" width="20" height="8" rx="2"/><rect x="2" y="14" width="20" height="8" rx="2"/></svg><span>نودها</span></a>' +
'<a class="nv" data-tab="banned" data-perm="advanced"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="4.93" y1="4.93" x2="19.07" y2="19.07"/></svg><span>IPهای بسته</span></a>' +
'<div class="nav-group">اتوماسیون</div>' +
'<a class="nv" data-tab="workflows" data-perm="advanced"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 12h-4l-3 9L9 3l-3 9H2"/></svg><span>Workflows</span></a>' +
'<a class="nv" data-tab="cron" data-perm="cron"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg><span>Cron Jobs</span></a>' +
'<a class="nv" data-tab="webhooks" data-perm="webhooks"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 16.98h-5.99c-1.1 0-2.12.5-2.83 1.34A4.95 4.95 0 0 1 5 20a5 5 0 1 1 5-5"/><path d="M6 8h15a3 3 0 0 1 3 3v6"/></svg><span>Webhooks</span></a>' +
'<a class="nv" data-tab="crisis" data-perm="users"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2L2 22h20L12 2zM12 9v5M12 18v.01"/></svg><span>اعلان بحران</span></a>' +
'<div class="nav-group">شبکه</div>' +
'<a class="nv" data-tab="regions" data-perm="advanced"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10"/></svg><span>مناطق IP</span></a>' +
'<a class="nv" data-tab="isp" data-perm="advanced"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12.55a11 11 0 0 1 14.08 0M1.42 9a16 16 0 0 1 21.16 0M8.53 16.11a6 6 0 0 1 6.95 0"/></svg><span>قالب اپراتور</span></a>' +
'<a class="nv" data-tab="dns" data-perm="advanced"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M2 12h20M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10"/></svg><span>DNS Pool</span></a>' +
'<a class="nv" data-tab="upstreams" data-perm="advanced"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2v20M2 12h20M4.93 4.93l14.14 14.14M19.07 4.93L4.93 19.07"/></svg><span>Upstreams</span></a>' +
'<a class="nv" data-tab="speedtest" data-perm="advanced"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 20v-6M6 20V10M18 20V4"/></svg><span>تست سرعت</span></a>' +
'<a class="nv" data-tab="latency" data-perm="stats"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 3v18h18"/><path d="M3 12h18M3 6h18M3 18h18"/></svg><span>Latency Map</span></a>' +
'<a class="nv" data-tab="dpi" data-perm="advanced"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg><span>DPI Detection</span></a>' +
'<div class="nav-group">تنظیمات</div>' +
'<a class="nv" data-tab="settings" data-perm="settings"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33"/></svg><span>تنظیمات</span></a>' +
'<a class="nv" data-tab="advanced" data-perm="advanced"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5"/></svg><span>پیشرفته</span></a>' +
'<a class="nv" data-tab="apikeys" data-perm="apikeys"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 2l-2 2"/></svg><span>API Keys</span></a>' +
'<a class="nv" data-tab="backup" data-perm="backup"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/></svg><span>پشتیبان</span></a>' +
'<a class="nv" data-tab="logs" data-perm="logs"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/></svg><span>لاگ‌ها</span></a>' +
'</nav><div class="dvd"></div>' +
...
'<button class="btn btn-g" style="width:100%;margin-top:12px" onclick="lo()">خروج</button>' +
...
'<div class="ph"><div><h1 class="pt">داشبورد</h1><p class="ps">نمای کلی سیستم</p></div><div><span class="badge-live" id="liveBadge">● live</span> <button class="btn btn-g btn-s" onclick="ls()">↺</button></div></div>' +
'<div class="sg">' +
'<div class="sc" style="animation-delay:0s"><div class="sl">👥 کاربران</div><div class="sv" id="st1">0</div></div>' +
'<div class="sc" style="animation-delay:.05s"><div class="sl">🟢 فعال</div><div class="sv" id="st2" style="color:var(--ok)">0</div></div>' +
'<div class="sc" style="animation-delay:.1s"><div class="sl">⏸️ متوقف</div><div class="sv" id="st3" style="color:var(--warn)">0</div></div>' +
'<div class="sc" style="animation-delay:.15s"><div class="sl">🔴 منقضی</div><div class="sv" id="st4" style="color:var(--danger)">0</div></div>' +
'<div class="sc" style="animation-delay:.2s"><div class="sl">📊 ترافیک کل</div><div class="sv" id="st5">0 <small>GB</small></div></div>' +
'<div class="sc" style="animation-delay:.25s"><div class="sl">📅 امروز</div><div class="sv" id="st6" style="color:var(--info)">0 <small>GB</small></div></div>' +
'<div class="sc" style="animation-delay:.3s"><div class="sl">🔌 اتصالات</div><div class="sv" id="st7">0</div></div>' +
'<div class="sc" style="animation-delay:.35s"><div class="sl">⏱ آپتایم</div><div class="sv" id="st8" style="font-size:20px">0h</div></div>' +
'</div>' +
'<div class="cg"><div class="cc"><div class="ct">📈 ترافیک ۷ روز</div><div class="cw"><canvas id="ch1"></canvas></div></div>' +
'<div class="cc"><div class="ct">🥧 توزیع امروز</div><div class="cw"><canvas id="ch2"></canvas></div></div></div>' +
'<div class="cc"><div class="ct">🏆 پرمصرف‌ترین‌ها</div><div id="topUsers"></div></div>' +
'</div>' +
/* WEATHER */
'<div id="tab-weather" class="tp">' +
'<div class="ph"><div><h1 class="pt">وضعیت شبکه</h1><p class="ps">Network Weather</p></div><button class="btn btn-g btn-s" onclick="lweather()">↺</button></div>' +
'<div id="weatherBox" class="gl" style="padding:24px"><div class="empty">در حال بارگذاری...</div></div>' +
'</div>' +
/* USERS */
'<div id="tab-users" class="tp">' +
'<div class="ph"><div><h1 class="pt">کاربران</h1><p class="ps">مدیریت مشترکین</p></div><div style="display:flex;gap:6px;flex-wrap:wrap"><button class="btn btn-g btn-s" onclick="exportCsv()">📥 Export</button><button class="btn btn-g btn-s" onclick="openImport()">📤 Import</button><button class="btn btn-p btn-s" onclick="ou()">+ جدید</button></div></div>' +
'<div style="margin-bottom:12px"><input id="userSearch" class="fld" placeholder="🔍 جستجو..." oninput="ru()" style="max-width:340px"></div>' +
'<div class="tw"><table><thead><tr><th>نام</th><th>وضعیت</th><th>مصرف</th><th>انقضا</th><th style="text-align:left">عملیات</th></tr></thead><tbody id="ub"><tr><td colspan="5" class="empty">در حال بارگذاری...</td></tr></tbody></table></div>' +
'</div>' +
/* GROUPS */
'<div id="tab-groups" class="tp">' +
'<div class="ph"><div><h1 class="pt">گروه‌ها</h1><p class="ps">پلن‌های پیش‌فرض</p></div><button class="btn btn-p btn-s" onclick="og()">+ گروه جدید</button></div>' +
'<div id="groupsGrid" class="card2"></div>' +
'</div>' +
/* TRAFFIC */
'<div id="tab-traffic" class="tp">' +
'<div class="ph"><div><h1 class="pt">ترافیک</h1><p class="ps">تحلیل دقیق</p></div></div>' +
'<div style="display:flex;gap:6px;margin-bottom:16px;flex-wrap:wrap"><div class="pill on" data-days="7" onclick="setDays(7,this)">۷ روز</div><div class="pill" data-days="14" onclick="setDays(14,this)">۱۴ روز</div><div class="pill" data-days="30" onclick="setDays(30,this)">۳۰ روز</div></div>' +
...
'<div class="cc"><div class="ct">🎯 مقایسه دوره‌ای</div><div class="fr" style="margin-bottom:12px"><div class="fd"><label>ID کاربران (با کاما)</label><input id="cmpIds" placeholder="u_abc,u_def"></div><div class="fd"><label>روز</label><input id="cmpDays" type="number" value="14"></div></div><button class="btn btn-p btn-s" onclick="runCompare()">مقایسه</button><div class="cw" style="height:300px;margin-top:12px"><canvas id="ch5"></canvas></div></div>' +
'</div>' +
/* ANOMALIES */
'<div id="tab-anomalies" class="tp">' +
'<div class="ph"><div><h1 class="pt">هشدارها</h1><p class="ps">مصرف غیرعادی</p></div><button class="btn btn-g btn-s" onclick="lanom()">↺</button></div>' +
'<div id="anomList" class="gl" style="padding:18px"><div class="empty">در حال بررسی...</div></div>' +
'</div>' +
/* PREDICTIVE */
'<div id="tab-predictive" class="tp">' +
'<div class="ph"><div><h1 class="pt">پیش‌بینی</h1><p class="ps">Predictive Analytics</p></div><button class="btn btn-g btn-s" onclick="lpredictive()">↺</button></div>' +
'<div id="predictiveBox" class="gl" style="padding:18px;margin-bottom:16px"><div class="empty">در حال محاسبه...</div></div>' +
'<div class="cc"><div class="ct">📈 نمودار پیش‌بینی</div><div class="cw"><canvas id="chPred"></canvas></div></div>' +
'</div>' +
/* SUGGESTIONS */
'<div id="tab-suggestions" class="tp">' +
'<div class="ph"><div><h1 class="pt">پیشنهادات</h1><p class="ps">Smart Suggestions</p></div><button class="btn btn-g btn-s" onclick="lsug()">↺</button></div>' +
'<div id="sugList" class="fg"></div>' +
'</div>' +
/* MANAGERS */
'<div id="tab-managers" class="tp">' +
'<div class="ph"><div><h1 class="pt">مدیران</h1><p class="ps">مدیریت دسترسی</p></div><button class="btn btn-p btn-s" onclick="om()">+ مدیر</button></div>' +
...
'<div class="ph"><div><h1 class="pt">نشست‌ها</h1><p class="ps">جلسات فعال</p></div><button class="btn btn-d btn-s" onclick="revokeAll()">🚫 قطع همه</button></div>' +
...
'<div class="ph"><div><h1 class="pt">نودها</h1><p class="ps">پنل‌های متصل</p></div><div style="display:flex;gap:6px"><button class="btn btn-g btn-s" onclick="healthAll()">🩺 سلامت</button><button class="btn btn-p btn-s" onclick="onNode()">+ نود</button></div></div>' +
'<div id="nodesGrid" class="card2"></div>' +
'</div>' +
/* BANNED */
'<div id="tab-banned" class="tp">' +
'<div class="ph"><div><h1 class="pt">IPهای بسته</h1><p class="ps">آی‌پی‌های مسدود شده</p></div><div style="display:flex;gap:6px;flex-wrap:wrap"><button class="btn btn-g btn-s" onclick="lbanned()">↺</button><button class="btn btn-p btn-s" onclick="oban()">+ Ban</button><button class="btn btn-d btn-s" onclick="clearBanned()">🗑️ پاک همه</button></div></div>' +
...
'<div class="ph"><div><h1 class="pt">Workflows</h1><p class="ps">اتوماسیون خودکار</p></div><button class="btn btn-p btn-s" onclick="owf()">+ Workflow</button></div>' +
'<div id="wfGrid" class="card2"></div>' +
'</div>' +
/* CRON */
'<div id="tab-cron" class="tp">' +
'<div class="ph"><div><h1 class="pt">Cron Jobs</h1><p class="ps">تسک‌های زمان‌بندی‌شده</p></div><button class="btn btn-p btn-s" onclick="ocron()">+ Job</button></div>' +
'<div id="cronGrid" class="card2"></div>' +
'</div>' +
/* WEBHOOKS */
'<div id="tab-webhooks" class="tp">' +
'<div class="ph"><div><h1 class="pt">Webhooks</h1><p class="ps">اعلان به سرویس‌های خارجی</p></div><button class="btn btn-p btn-s" onclick="owh()">+ Webhook</button></div>' +
'<div id="whGrid" class="card2"></div>' +
'</div>' +
/* CRISIS */
'<div id="tab-crisis" class="tp">' +
'<div class="ph"><div><h1 class="pt">اعلان بحران</h1><p class="ps">ارسال پیام فوری</p></div></div>' +
'<div class="gl" style="padding:20px;margin-bottom:16px"><div class="stt">⚡ پیام فوری</div>' +
...
'<div class="gl" style="padding:20px;margin-bottom:16px"><div class="stt">⚡ پریست‌ها</div><div id="crisisPresets" class="card2"></div></div>' +
'<div class="gl" style="padding:20px"><div class="stt">📜 تاریخچه</div><div id="crisisHistory"></div></div>' +
'</div>' +
/* REGIONS */
'<div id="tab-regions" class="tp">' +
'<div class="ph"><div><h1 class="pt">مناطق IP</h1><p class="ps">Clean IP Regions</p></div><div style="display:flex;gap:6px"><button class="btn btn-g btn-s" onclick="runCleanIp()">⚡ تست</button><button class="btn btn-p btn-s" onclick="oregion()">+ منطقه</button></div></div>' +
...
'<div class="gl" style="padding:18px"><div class="stt">📊 نتایج آخرین تست</div><div class="tw" style="border:none;padding:0"><table><thead><tr><th>IP</th><th>Ping</th></tr></thead><tbody id="cleanBody"><tr><td colspan="2" class="empty">هنوز تست نشده</td></tr></tbody></table></div></div>' +
'</div>' +
/* ISP */
'<div id="tab-isp" class="tp">' +
'<div class="ph"><div><h1 class="pt">قالب اپراتور</h1><p class="ps">ISP Templates</p></div><button class="btn btn-p btn-s" onclick="saveIsp()">💾 ذخیره</button></div>' +
'<div id="ispGrid" class="fg"></div>' +
'</div>' +
/* DNS */
'<div id="tab-dns" class="tp">' +
'<div class="ph"><div><h1 class="pt">DNS Pool</h1><p class="ps">استخر DNS</p></div><div style="display:flex;gap:6px"><button class="btn btn-g btn-s" onclick="dnsTest()">⚡ تست</button><button class="btn btn-p btn-s" onclick="saveDns()">💾 ذخیره</button></div></div>' +
'<div class="gl" style="padding:18px;margin-bottom:16px"><div class="fd"><label>استراتژی انتخاب</label><select id="dnsStrategy"><option value="weighted">Weighted (وزنی)</option><option value="random">Random</option></select></div></div>' +
'<div id="dnsGrid" class="fg"></div>' +
'</div>' +
/* UPSTREAMS */
'<div id="tab-upstreams" class="tp">' +
'<div class="ph"><div><h1 class="pt">Upstreams</h1><p class="ps">سرورهای زنجیره‌ای</p></div><button class="btn btn-p btn-s" onclick="oupstream()">+ Upstream</button></div>' +
'<div id="upstreamGrid" class="fg"></div>' +
'</div>' +
/* SPEEDTEST */
'<div id="tab-speedtest" class="tp">' +
'<div class="ph"><div><h1 class="pt">تست سرعت</h1><p class="ps">Speed Test</p></div><button class="btn btn-p btn-s" onclick="runSpeed()">▶️ اجرا</button></div>' +
'<div class="tw"><table><thead><tr><th>Host</th><th>Latency</th><th>Status</th></tr></thead><tbody id="speedBody"><tr><td colspan="3" class="empty">هنوز تست نشده</td></tr></tbody></table></div>' +
'</div>' +
/* LATENCY */
'<div id="tab-latency" class="tp">' +
'<div class="ph"><div><h1 class="pt">Latency Map</h1><p class="ps">نقشه تاخیر</p></div><button class="btn btn-g btn-s" onclick="llatency()">↺</button></div>' +
'<div id="latencyGrid" class="fg"></div>' +
'</div>' +
/* DPI */
'<div id="tab-dpi" class="tp">' +
'<div class="ph"><div><h1 class="pt">DPI Detection</h1><p class="ps">تشخیص DPI</p></div><button class="btn btn-p btn-s" onclick="runDpi()">🔍 بررسی</button></div>' +
'<div id="dpiBox" class="gl" style="padding:24px"><div class="empty">برای بررسی کلیک کنید</div></div>' +
'</div>' +
/* SETTINGS */
'<div id="tab-settings" class="tp">' +
'<div class="ph"><div><h1 class="pt">تنظیمات</h1><p class="ps">پیکربندی اصلی</p></div></div>' +
...
'<div class="stt" style="margin-top:20px">🎨 لوگوی اختصاصی</div>' +
'<div class="fd"><label>Base64 یا URL (حداکثر 200KB)</label><input id="c9" placeholder="data:image/png;base64,..."></div>' +
'<div class="fd"><label>رنگ عنوان</label><input id="c10" placeholder="#8b5cf6"></div>' +
'</div><div style="display:flex;gap:10px;margin-top:20px;flex-wrap:wrap"><button class="btn btn-p" onclick="sc()">💾 ذخیره</button><button class="btn btn-g" onclick="fc()">↺</button><button class="btn btn-d" onclick="clearLogo()">🗑️ حذف لوگو</button></div></div>' +
'</div>' +
/* ADVANCED */
'<div id="tab-advanced" class="tp">' +
'<div class="ph"><div><h1 class="pt">تنظیمات پیشرفته</h1></div></div>' +
...
'</div></div>' +
'<div><button class="btn btn-p" onclick="sc()">💾 ذخیره</button></div></div>' +
'</div>' +
/* APIKEYS */
'<div id="tab-apikeys" class="tp">' +
'<div class="ph"><div><h1 class="pt">API Keys</h1></div><button class="btn btn-p btn-s" onclick="ck()">+ کلید</button></div>' +
...
'<div class="ph"><div><h1 class="pt">پشتیبان</h1></div><div style="display:flex;gap:6px;flex-wrap:wrap"><button class="btn btn-g btn-s" onclick="exportConfig()">📥 JSON</button><button class="btn btn-g btn-s" onclick="openImportConfig()">📤 Import</button><button class="btn btn-p btn-s" onclick="makeBackup()">💾 R2</button></div></div>' +
'<div class="tw"><table><thead><tr><th>فایل</th><th>اندازه</th><th>تاریخ</th><th style="text-align:left">عملیات</th></tr></thead><tbody id="backupBody"><tr><td colspan="4" class="empty">در حال بارگذاری...</td></tr></tbody></table></div>' +
'</div>' +
/* LOGS */
'<div id="tab-logs" class="tp">' +
'<div class="ph"><div><h1 class="pt">لاگ‌ها</h1></div><button class="btn btn-g btn-s" onclick="ll()">↺</button></div>' +
'<div class="gl" style="padding:18px"><div id="lc" style="display:flex;flex-direction:column;gap:8px"></div></div>' +
'</div>' +
'</main></div>' +
/* MODALS */
...
'<div id="gm" class="mb" style="display:none"><div class="md">' +
...
'<div class="fr"><div class="fd"><label>ترافیک (GB)</label><input id="g2" type="number" placeholder="0=∞"></div><div class="fd"><label>روزانه (GB)</label><input id="g3" type="number" placeholder="0=∞"></div></div>' +
'<div class="fr"><div class="fd"><label>اعتبار (روز)</label><input id="g4" type="number" placeholder="0=∞"></div><div class="fd"><label>حداکثر کانفیگ</label><input id="g5" type="number" placeholder="0=∞"></div></div>' +
'<div class="fd"><label>محدودیت اتصال</label><input id="g6" type="number" placeholder="0=∞"></div>' +
'<div style="display:flex;gap:10px;margin-top:8px"><button class="btn btn-p" style="flex:1" onclick="sg2()">💾</button><button class="btn btn-g" onclick="cg()">انصراف</button></div></div></div></div>' +
...
'<div style="display:flex;gap:10px;margin-top:8px"><button class="btn btn-p" style="flex:1" onclick="sm()">💾</button><button class="btn btn-g" onclick="cm()">انصراف</button></div></div></div></div>' +
...
'<div style="display:flex;gap:10px;margin-top:8px"><button class="btn btn-p" style="flex:1" onclick="sn()">💾</button><button class="btn btn-g" onclick="cn()">انصراف</button></div></div></div></div>' +
...
'<div style="display:flex;gap:10px;margin-top:8px"><button class="btn btn-p" style="flex:1" onclick="sr2()">💾</button><button class="btn btn-g" onclick="cr()">انصراف</button></div></div></div></div>' +
...
'<div style="display:flex;gap:10px;margin-top:8px"><button class="btn btn-p" style="flex:1" onclick="scj()">💾</button><button class="btn btn-g" onclick="ccj()">انصراف</button></div></div></div></div>' +
...
'<div style="display:flex;gap:10px;margin-top:8px"><button class="btn btn-p" style="flex:1" onclick="swh()">💾</button><button class="btn btn-g" onclick="cwh()">انصراف</button></div></div></div></div>' +
...
'<div class="fg"><div class="fd"><label>IP</label><input id="b1" placeholder="1.2.3.4"></div>' +
...
'<div class="fd"><label>Actions (JSON آرایه)</label><textarea id="wf3" rows="5">[{"type":"send.telegram","params":{"message":"سلام {name}"}}]</textarea></div>' +
'<div style="display:flex;gap:10px;margin-top:8px"><button class="btn btn-p" style="flex:1" onclick="swf()">💾</button><button class="btn btn-g" onclick="cwf()">انصراف</button></div></div></div></div>' +
...
'<div style="display:flex;gap:10px;margin-top:8px"><button class="btn btn-p" style="flex:1" onclick="sup()">💾</button><button class="btn btn-g" onclick="cup()">انصراف</button></div></div></div></div>' +
...
'<div class="fd"><label>محتوای CSV</label><textarea id="csvData" rows="10" placeholder="name,groupId,isp,trafficLimitGB,dailyLimitGB,expiryDays,maxConfigs,connLimit,bandwidthKbps,notes"></textarea></div>' +
'<div style="display:flex;gap:10px;margin-top:12px"><button class="btn btn-p" style="flex:1" onclick="doImport()">📤</button><button class="btn btn-g" onclick="ci()">انصراف</button></div></div></div>' +
...
'<div style="display:flex;gap:10px;margin-top:12px"><button class="btn btn-p" style="flex:1" onclick="doImportConfig()">📤</button><button class="btn btn-g" onclick="cic()">انصراف</button></div></div></div>' +
...
'let S={token:null,me:null,config:null,users:[],managers:[],groups:[],sessions:[],nodes:[],regions:[],activeRegions:[],cronJobs:[],cronActions:[],webhooks:[],webhookEvents:[],banned:[],crisisPresets:[],crisisHistory:[],ispTemplates:{},workflows:[],workflowTriggers:[],workflowActions:[],upstreams:[],dnsPool:[],dnsStrategy:"weighted",stats:null,editU:null,editG:null,editM:null,editCron:null,editWh:null,editRegion:null,editWf:null,days:7,pollTimer:null,charts:{}};' +
'const $=id=>document.getElementById(id);' +
'function ts(m,t){t=t||"info";const c={info:"#38bdf8",ok:"#10b981",warn:"#f59e0b",error:"#ef4444"}[t],i={info:"ℹ️",ok:"✅",warn:"⚠️",error:"❌"}[t];const e=document.createElement("div");e.className="tst";e.style.borderLeft="3px solid "+c;e.innerHTML="<span>"+i+"</span><span>"+m+"</span>";$("tb").appendChild(e);setTimeout(()=>{e.style.opacity="0";e.style.transition=".3s";setTimeout(()=>e.remove(),300)},3000)}' +
...
'function showLoginError(msg){const el=$("le");el.style.display="block";el.textContent=msg}' +
'async function login(){const u=$("lu").value.trim(),p=$("lp").value;if(!u||!p){showLoginError("نام و رمز الزامی");return}const b=$("lb");b.disabled=true;b.innerHTML="⏳ در حال ورود...";$("le").style.display="none";try{const r=await fetch("/"+AR+"/api/auth",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:u,password:p})});let d;try{d=await r.json()}catch(pe){showLoginError("پاسخ نامعتبر ("+r.status+")");b.disabled=false;b.innerHTML="🔓 ورود به پنل";return}if(d.ok||d.success){const sess=(d.data&&d.data.session)||d.session;const cfg=(d.data&&d.data.config)||d.config;if(!sess||!sess.token){showLoginError("توکن دریافت نشد");b.disabled=false;b.innerHTML="🔓 ورود به پنل";return}S.token=sess.token;S.me=sess;S.config=cfg||{};sessionStorage.setItem("hp_token",S.token);sessionStorage.setItem("hp_user",u);show()}else{showLoginError(d.error||"خطا در ورود");b.disabled=false;b.innerHTML="🔓 ورود به پنل"}}catch(e){showLoginError("خطا در اتصال: "+(e.message||"network"));b.disabled=false;b.innerHTML="🔓 ورود به پنل"}}' +
'function show(){$("login").style.display="none";$("shell").classList.add("on");$("whoami").textContent=(S.me.isRoot?"👑 ":"👤 ")+S.me.username;renderLogo();applyPerms();fc();refreshAll();startPolling()}' +
...
'function applyPerms(){const has=p=>S.me.isRoot||S.me.permissions.includes("all")||S.me.permissions.includes(p);document.querySelectorAll("[data-perm]").forEach(el=>{if(!has(el.dataset.perm))el.style.display="none"})}' +
...
'document.querySelectorAll(".nv").forEach(n=>n.addEventListener("click",()=>tab(n.dataset.tab)));' +
'function startPolling(){if(S.pollTimer)clearInterval(S.pollTimer);S.pollTimer=setInterval(()=>{if(document.hidden)return;const t=document.querySelector(".tp.on");if(!t)return;const id=t.id;if(id==="tab-overview")ls()},15000)}' +
...
'async function clearLogo(){if(!confirm("حذف؟"))return;const r=await ap("/api/logo",{method:"POST",body:JSON.stringify({action:"clear"})});if(r.data.ok||r.data.success){ts("حذف شد","ok");S.config.customLogo="";renderLogo()}}' +
...
'let tp="";(s.system.topUsers||[]).forEach((u,i)=>{const pct=u.gb>0?Math.min(100,(u.gb/Math.max(1,s.traffic.totalGB))*100):0;tp+=\'<div style="padding:10px;border-radius:12px;background:rgba(148,163,184,.05);margin-bottom:6px;display:flex;align-items:center;gap:10px"><span style="font-size:18px">\'+(i===0?"🥇":i===1?"🥈":i===2?"🥉":"🏅")+\'</span><div style="flex:1"><div style="font-weight:700;font-size:13px">\'+u.name+\'</div><div class="prg"><div style="width:\'+pct.toFixed(1)+\'%"></div></div></div><div style="font-weight:800;color:var(--c2)">\'+u.gb+\' GB</div></div>\'});$("topUsers").innerHTML=tp||\'<div class="empty">داده نیست</div>\';setTimeout(()=>{c1();c2()},100)}' +
'async function lu2(){const r=await ap("/api/users");if(!(r.data.ok||r.data.success))return;S.users=r.data.data||r.data.users||[];ru()}' +
'function bd(s){const m={active:["bdg-ok","🟢"],paused:["bdg-w","⏸️"],expired:["bdg-d","🔴"],"auto-disabled":["bdg-d","🚫"]};const v=m[s]||["bdg-m",s];return \'<span class="bdg \'+v[0]+\'">\'+v[1]+\'</span>\'}' +
'function fb(b){if(!b)return"0 GB";const g=b/1073741824;if(g<1)return(b/1048576).toFixed(1)+" MB";return g.toFixed(2)+" GB"}' +
'function ru(){const q=($("userSearch")?.value||"").toLowerCase();let u=S.users;if(q)u=u.filter(x=>x.name.toLowerCase().includes(q)||x.id.toLowerCase().includes(q));if(!u.length){$("ub").innerHTML=\'<tr><td colspan="5" class="empty">خالی</td></tr>\';return}let h="";u.forEach(x=>{const us=x.usage?fb(x.usage.total):"0 GB",lm=x.limitTotalReq?fb(x.limitTotalReq*1073741824/6000):"∞",ex=x.expiryMs?new Date(x.expiryMs).toLocaleDateString("fa-IR"):"∞";let pct=0;if(x.limitTotalReq&&x.usage&&x.usage.total){const lb=x.limitTotalReq*1073741824/6000;pct=Math.min(100,(x.usage.total/lb)*100)}const pc=pct>90?"d":pct>70?"w":"";const grp=(S.config.userGroups||[]).find(g=>g.id===(x.groupId||"default"));h+=\'<tr><td><div style="font-weight:800">\'+(x.name||"—")+\'</div>\'+(grp?\'<span class="tag-mini" style="background:\'+grp.color+\'22;color:\'+grp.color+\'">\'+grp.name+\'</span>\':\'\')+\'</td><td>\'+bd(x.status)+\'</td><td><div style="font-weight:700;font-size:12px">\'+us+\'</div><div style="font-size:10px;color:var(--muted)">/ \'+lm+\'</div><div class="prg \'+pc+\'"><div style="width:\'+pct.toFixed(1)+\'%"></div></div></td><td style="font-size:12px">\'+ex+\'</td><td style="text-align:left"><button class="btn btn-g btn-s" onclick="sun(\\\'\'+x.id+\'\\\')" style="margin-left:4px" title="ساب">🔗</button><button class="btn btn-g btn-s" onclick="tu(\\\'\'+x.id+\'\\\')" style="margin-left:4px">\'+(x.isPaused?"▶️":"⏸️")+\'</button><button class="btn btn-g btn-s" onclick="eu(\\\'\'+x.id+\'\\\')" style="margin-left:4px">✏️</button><button class="btn btn-d btn-s" onclick="du(\\\'\'+x.id+\'\\\')">🗑️</button></td></tr>\'});$("ub").innerHTML=h}' +
...
'function ou(){S.editU=null;$("umt").textContent="کاربر جدید";["u1","u2","u3","u4","u5","u7","u10"].forEach(k=>{if($(k))$(k).value=""});$("u8").value="none";refreshGroupSelect();$("um").style.display="flex"}' +
...
'function eu(id){const u=S.users.find(x=>x.id===id);if(!u)return;S.editU=u;$("umt").textContent="ویرایش: "+u.name;$("u1").value=u.name||"";$("u2").value=u.limitTotalReq?(u.limitTotalReq/6000).toFixed(2):"";$("u3").value=u.limitDailyReq?(u.limitDailyReq/6000).toFixed(2):"";$("u4").value=u.expiryMs?Math.max(0,Math.ceil((u.expiryMs-Date.now())/86400000)):"";$("u5").value=u.notes||"";$("u7").value=u.maxConfigs||"";$("u9").value=u.isp||"";$("u10").value=u.bandwidthKbps||"";const c=(S.config.autoResetCycles||{})[u.id];$("u8").value=c?c.type:"none";refreshGroupSelect();$("u6").value=u.groupId||"default";$("um").style.display="flex"}' +
'async function su2(){const n=$("u1").value.trim();if(!n){ts("نام الزامی","warn");return}const p={name:n,groupId:$("u6").value,isp:$("u9").value||null,trafficLimit:$("u2").value||0,dailyLimit:$("u3").value||0,expiryDays:$("u4").value||0,notes:$("u5").value,maxConfigs:$("u7").value||0,bandwidthKbps:$("u10").value||0,autoReset:{type:$("u8").value}};let r;if(S.editU)r=await ap("/api/users?id="+encodeURIComponent(S.editU.id),{method:"PUT",body:JSON.stringify(p)});else r=await ap("/api/users",{method:"POST",body:JSON.stringify(p)});if(r.data.ok||r.data.success){ts("ذخیره","ok");cu();lu2();ls()}else ts("خطا","error")}' +
...
'function rg(){const g=S.groups;if(!g.length){$("groupsGrid").innerHTML=\'<div class="empty">خالی</div>\';return}let h="";g.forEach(x=>{h+=\'<div class="node-card"><div style="font-size:15px;font-weight:900;margin-bottom:8px"><span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:\'+x.color+\';margin-left:6px"></span>\'+x.name+\'</div><div style="font-size:11px;color:var(--muted);line-height:1.8">📊 \'+(x.limitTotalGb||"∞")+\' GB · 📅 \'+(x.expiryDays||"∞")+\'</div><div style="display:flex;gap:6px;margin-top:10px">\'+(x.id!=="default"?\'<button class="btn btn-g btn-s" onclick="eg(\\\'\'+x.id+\'\\\')">✏️</button><button class="btn btn-d btn-s" onclick="dg(\\\'\'+x.id+\'\\\')">🗑️</button>\':\'<span class="tag-mini">پیش‌فرض</span>\')+\'</div></div>\'});$("groupsGrid").innerHTML=h}' +
'function og(){S.editG=null;$("gmt").textContent="گروه";["g1","g2","g3","g4","g5","g6"].forEach(k=>$(k).value="");$("gm").style.display="flex"}' +
'function cg(){$("gm").style.display="none"}' +
'function eg(id){const g=S.groups.find(x=>x.id===id);if(!g)return;S.editG=g;$("gmt").textContent="ویرایش";$("g1").value=g.name;$("g2").value=g.limitTotalGb||"";$("g3").value=g.limitDailyGb||"";$("g4").value=g.expiryDays||"";$("g5").value=g.maxConfigs||"";$("g6").value=g.connLimit||"";$("gm").style.display="flex"}' +
...
'function rm(){const m=S.managers;if(!m.length){$("mb").innerHTML=\'<tr><td colspan="5" class="empty">خالی</td></tr>\';return}let h="";m.forEach(x=>{const pm=x.isRoot?\'<span class="bdg bdg-i">👑 Root</span>\':\'<span class="bdg bdg-m">\'+(x.permissions||[]).length+\'</span>\';const st=x.isActive!==false?\'<span class="bdg bdg-ok">🟢</span>\':\'<span class="bdg bdg-d">🔴</span>\';const ll=x.lastLogin?new Date(x.lastLogin).toLocaleDateString("fa-IR"):"—";h+=\'<tr><td style="font-weight:800">\'+x.username+\'</td><td>\'+pm+\'</td><td>\'+st+\'</td><td style="font-size:12px">\'+ll+\'</td><td style="text-align:left">\';if(!x.isRoot)h+=\'<button class="btn btn-g btn-s" onclick="em(\\\'\'+x.id+\'\\\')" style="margin-left:4px">✏️</button><button class="btn btn-d btn-s" onclick="dm(\\\'\'+x.id+\'\\\')">🗑️</button>\';else h+="—";h+="</td></tr>"});$("mb").innerHTML=h}' +
'const ALLP=["users","settings","advanced","managers","apikeys","logs","stats","subscriptions","nodes","backup","groups","cron","webhooks","regions"];' +
...
'async function lsess(){const r=await ap("/api/sessions");const b=$("sessBody");if(!(r.data.ok||r.data.success)){b.innerHTML=\'<tr><td colspan="4" class="empty">دسترسی نیست</td></tr>\';return}const ss=r.data.data||r.data.sessions||[];if(!ss.length){b.innerHTML=\'<tr><td colspan="4" class="empty">خالی</td></tr>\';return}let h="";ss.forEach(s=>{h+=\'<tr><td style="font-weight:700">\'+s.username+(s.isRoot?\' <span class="bdg bdg-i">👑</span>\':\'\')+(s.current?\' <span class="bdg bdg-ok">فعلی</span>\':\'\')+\'</td><td class="mono">\'+s.ip+\'</td><td style="font-size:11px">\'+new Date(s.createdAt).toLocaleString("fa-IR")+\'</td><td style="text-align:left">\'+(s.current?"—":\'<button class="btn btn-d btn-s" onclick="rs(\\\'\'+s.fullToken+\'\\\')">قطع</button>\')+\'</td></tr>\'});b.innerHTML=h}' +
...
'function rn(){const n=S.nodes;if(!n.length){$("nodesGrid").innerHTML=\'<div class="empty">نودی نیست</div>\';return}let h="";n.forEach(x=>{const hh=x.lastHealth||{};const st=hh.status==="online"?"🟢 آنلاین":hh.status==="offline"?"🔴 آفلاین":hh.status?"🟡 خطا":"⚪ تست‌نشده";h+=\'<div class="node-card"><div style="font-weight:800;font-size:13px;margin-bottom:6px">\'+(x.name||x.url)+\'</div><div class="mono" style="font-size:10px;margin-bottom:8px;word-break:break-all">\'+x.url+\'</div><div style="font-size:12px">\'+st+(hh.latency>=0?\' · \'+hh.latency+\'ms\':\'\')+\'</div><button class="btn btn-d btn-s" style="margin-top:10px" onclick="dnode(\\\'\'+x.url+\'\\\')">🗑️</button></div>\'});$("nodesGrid").innerHTML=h}' +
...
'function oregion(){$("rmt").textContent="منطقه جدید";$("r1").value="";$("r2").value="";$("r3").value="";$("rm").style.display="flex"}' +
...
'async function lanom(){const r=await ap("/api/anomalies");if(!(r.data.ok||r.data.success))return;const a=r.data.data||r.data.anomalies||[];const el=$("anomList");if(!a.length){el.innerHTML=\'<div class="empty">هیچ هشدار ✨</div>\';return}let h="";a.forEach(x=>{h+=\'<div style="padding:14px;border-radius:12px;background:rgba(239,68,68,.08);border:1px solid rgba(239,68,68,.3);margin-bottom:8px;display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap"><div><div style="font-weight:800">\'+x.name+\'</div><div style="font-size:11px;color:var(--muted);margin-top:4px">امروز: \'+(x.today/6000).toFixed(2)+\' GB · میانگین: \'+(x.avg/6000).toFixed(2)+\' GB</div></div><span class="bdg bdg-d">×\'+x.ratio+\'</span></div>\'});el.innerHTML=h}' +
...
'function rc(){const j=S.cronJobs;if(!j.length){$("cronGrid").innerHTML=\'<div class="empty">خالی</div>\';return}let h="";j.forEach(x=>{const st=x.lastStatus==="ok"?\'<span class="bdg bdg-ok">✓</span>\':x.lastStatus==="error"?\'<span class="bdg bdg-d">✗</span>\':\'\';h+=\'<div class="node-card"><div style="font-weight:800;margin-bottom:6px">\'+x.name+\' \'+st+\'</div><div style="font-size:11px;color:var(--muted);line-height:1.7">⚡ \'+x.action+\'<br>⏰ هر \'+x.intervalMinutes+\' دقیقه</div><div style="display:flex;gap:6px;margin-top:10px"><button class="btn btn-p btn-s" onclick="runCron(\\\'\'+x.id+\'\\\')">▶️</button><button class="btn btn-d btn-s" onclick="dcron(\\\'\'+x.id+\'\\\')">🗑️</button></div></div>\'});$("cronGrid").innerHTML=h}' +
...
'function rb(){const b=S.banned;if(!b.length){$("banBody").innerHTML=\'<tr><td colspan="4" class="empty">خالی</td></tr>\';return}let h="";b.forEach(x=>{h+=\'<tr><td class="mono">\'+x.ip+\'</td><td style="font-size:12px">\'+(x.reason||"—")+\'</td><td style="font-size:11px">\'+new Date(x.bannedAt).toLocaleString("fa-IR")+\'</td><td style="text-align:left"><button class="btn btn-g btn-s" onclick="unbanOne(\\\'\'+x.ip+\'\\\')">رفع</button></td></tr>\'});$("banBody").innerHTML=h}' +
...
'async function lup(){const r=await ap("/api/upstreams");if(!(r.data.ok||r.data.success))return;S.upstreams=r.data.data||[];let h="";if(!S.upstreams.length){$("upstreamGrid").innerHTML=\'<div class="empty">خالی</div>\';return}S.upstreams.forEach(x=>{h+=\'<div class="node-card"><div style="font-weight:800;margin-bottom:6px">\'+x.name+\' \'+(x.enabled?\'<span class="bdg bdg-ok">✓</span>\':\'<span class="bdg bdg-d">✗</span>\')+\'</div><div class="mono" style="font-size:10px;word-break:break-all">\'+x.uri.slice(0,60)+\'...</div><div style="display:flex;gap:6px;margin-top:10px"><button class="btn btn-g btn-s" onclick="togUp(\\\'\'+x.id+\'\\\')">\'+(x.enabled?"غیرفعال":"فعال")+\'</button><button class="btn btn-d btn-s" onclick="dup(\\\'\'+x.id+\'\\\')">🗑️</button></div></div>\'});$("upstreamGrid").innerHTML=h}' +
...
'async function llatency(){const r=await ap("/api/latency-map");if(!(r.data.ok||r.data.success))return;const d=r.data.data||{};let h="";Object.keys(d).forEach(k=>{const x=d[k];const pct=x.avgLatency?Math.min(100,Math.max(0,(x.avgLatency/500)*100)):0;const col=x.avgLatency<200?"var(--ok)":x.avgLatency<500?"var(--warn)":"var(--danger)";h+=\'<div class="node-card"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px"><div style="font-weight:800">\'+x.flag+\' \'+x.name+\'</div><div>\'+(x.active?\'<span class="bdg bdg-ok">فعال</span>\':\'<span class="bdg bdg-m">غیرفعال</span>\')+\'</div></div><div style="font-size:11px;color:var(--muted)">\'+x.ipCount+\' آی‌پی · \'+(x.avgLatency?x.avgLatency+" ms":"—")+\'</div>\'+(x.avgLatency?\'<div class="latency-bar"><div style="width:\'+pct+\'%;background:\'+col+\'"></div></div>\':\'\')+\'</div>\'});$("latencyGrid").innerHTML=h||\'<div class="empty">خالی</div>\'}' +
'async function runDpi(){const r=await ap("/api/dpi");if(!(r.data.ok||r.data.success))return;const d=r.data.data||{};const emoji=d.mode==="high"?"🚨":d.mode==="medium"?"⚠️":"✅";const col=d.mode==="high"?"var(--danger)":d.mode==="medium"?"var(--warn)":"var(--ok)";$("dpiBox").innerHTML=\'<div style="text-align:center;padding:20px"><div style="font-size:64px;margin-bottom:16px">\'+emoji+\'</div><div style="font-size:24px;font-weight:900;color:\'+col+\'">\'+d.mode.toUpperCase()+\'</div><div style="font-size:14px;color:var(--muted);margin-top:8px">امتیاز: \'+d.score+\'</div><div style="font-size:12px;color:var(--muted);margin-top:16px">\'+(d.reasons||[]).join(" · ")+\'</div><div style="font-size:11px;color:var(--muted);margin-top:8px">ASN: \'+d.asn+\' · کشور: \'+d.country+\'</div></div>\'}' +
'async function lweather(){const r=await ap("/api/network-weather");if(!(r.data.ok||r.data.success))return;const d=r.data.data||{};$("weatherBox").innerHTML=\'<div class="weather"><div class="weather-icon">\'+d.emoji+\'</div><div style="flex:1"><div style="font-size:20px;font-weight:900">\'+d.status.toUpperCase()+\'</div><div style="font-size:13px;color:var(--muted);margin-top:6px">سلامت: \'+d.health+\'% · کاربران فعال: \'+d.active+\'/\'+d.total+\'</div><div style="font-size:12px;color:var(--muted);margin-top:4px">نودهای آنلاین: \'+d.nodesOnline+\'/\'+d.nodesTotal+\'</div>\'+(d.cfUsage!==null?\'<div style="font-size:12px;color:var(--muted);margin-top:4px">CF: \'+d.cfUsage+\' requests (\'+d.cfPct+\'%)</div>\':\'\')+\'<div class="prg" style="margin-top:10px"><div style="width:\'+d.health+\'%"></div></div></div></div>\'}' +
'async function lpredictive(){const r=await ap("/api/predictive");if(!(r.data.ok||r.data.success))return;const d=r.data.data||{};$("predictiveBox").innerHTML=\'<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:14px"><div><div style="font-size:11px;color:var(--muted);text-transform:uppercase">میانگین روزانه</div><div style="font-size:22px;font-weight:900;color:var(--c2);margin-top:6px">\'+d.avg.toFixed(2)+\' GB</div></div><div><div style="font-size:11px;color:var(--muted);text-transform:uppercase">روند</div><div style="font-size:22px;font-weight:900;color:\'+(d.trend>0?"var(--danger)":"var(--ok)")+\';margin-top:6px">\'+(d.trend>0?"+":"")+d.trend.toFixed(3)+\'</div></div><div><div style="font-size:11px;color:var(--muted);text-transform:uppercase">پیش‌بینی هفته</div><div style="font-size:22px;font-weight:900;color:var(--info);margin-top:6px">\'+d.nextWeekTotal+\' GB</div></div></div>\';if(S.charts.chPred)S.charts.chPred.destroy();const cv=$("chPred");if(!cv)return;S.charts.chPred=new Chart(cv.getContext("2d"),{type:"line",data:{labels:(d.predictions||[]).map(x=>x.date.slice(5)),datasets:[{label:"پیش‌بینی (GB)",data:(d.predictions||[]).map(x=>x.gb),borderColor:"#ec4899",backgroundColor:"rgba(236,72,153,.15)",fill:true,tension:.4,borderWidth:3}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{labels:{color:"#b8c1d9"}}},scales:{x:{ticks:{color:"#7c88a6"},grid:{color:"rgba(148,163,184,.06)"}},y:{ticks:{color:"#7c88a6"},grid:{color:"rgba(148,163,184,.06)"},beginAtZero:true}}}})}' +
'async function lsug(){const r=await ap("/api/suggestions");if(!(r.data.ok||r.data.success))return;const a=r.data.data||r.data.suggestions||[];if(!a.length){$("sugList").innerHTML=\'<div class="empty">هیچ پیشنهادی ✨</div>\';return}let h="";a.forEach(x=>{const col=x.level==="warn"?"var(--warn)":"var(--info)";h+=\'<div class="suggestion"><div style="font-size:28px">\'+x.icon+\'</div><div style="flex:1"><div style="font-weight:800;font-size:14px;color:\'+col+\'">\'+x.title+\'</div><div style="font-size:12px;color:var(--muted);margin-top:4px">\'+x.desc+\'</div>\'+(x.action?\'<button class="btn btn-g btn-s" style="margin-top:8px" onclick="handleSug(\\\'\'+x.action+\'\\\')">برو</button>\':\'\')+\'</div></div>\'});$("sugList").innerHTML=h}' +
'function handleSug(a){if(a.startsWith("tab:"))tab(a.slice(4))}' +
'async function lk(){const r=await ap("/api/keys");const b=$("kb");if(!(r.data.ok||r.data.success)){b.innerHTML=\'<tr><td colspan="4" class="empty">دسترسی نیست</td></tr>\';return}const k=r.data.data||r.data.keys||[];if(!k.length){b.innerHTML=\'<tr><td colspan="4" class="empty">خالی</td></tr>\';return}let h="";k.forEach(x=>{h+=\'<tr><td>\'+(x.name||"—")+\'</td><td class="mono">\'+(x.keyPreview||"—")+\'</td><td style="font-size:12px">\'+(x.createdAt?new Date(x.createdAt).toLocaleDateString("fa-IR"):"—")+\'</td><td style="text-align:left"><button class="btn btn-d btn-s" onclick="rk(\\\'\'+x.id+\'\\\')">حذف</button></td></tr>\'});b.innerHTML=h}' +
...
'async function lbackup(){const r=await ap("/api/backup");if(!(r.data.ok||r.data.success)){$("backupBody").innerHTML=\'<tr><td colspan="4" class="empty">دسترسی نیست</td></tr>\';return}const b=r.data.data||r.data.backups||[];if(!b.length){$("backupBody").innerHTML=\'<tr><td colspan="4" class="empty">خالی</td></tr>\';return}let h="";b.forEach(x=>{const kb=(x.size/1024).toFixed(1);const n=x.key.split("/").pop();h+=\'<tr><td class="mono" style="font-size:10px">\'+n+\'</td><td>\'+kb+\' KB</td><td style="font-size:11px">\'+new Date(x.uploaded).toLocaleString("fa-IR")+\'</td><td style="text-align:left"><button class="btn btn-p btn-s" onclick="restoreB(\\\'\'+x.key+\'\\\')" style="margin-left:4px">بازیابی</button><button class="btn btn-d btn-s" onclick="delB(\\\'\'+x.key+\'\\\')">🗑️</button></td></tr>\'});$("backupBody").innerHTML=h}' +
...
'const cmds=[{l:"داشبورد",a:()=>tab("overview")},{l:"وضعیت شبکه",a:()=>tab("weather")},{l:"کاربران",a:()=>tab("users")},{l:"گروه‌ها",a:()=>tab("groups")},{l:"ترافیک",a:()=>tab("traffic")},{l:"هشدارها",a:()=>tab("anomalies")},{l:"پیش‌بینی",a:()=>tab("predictive")},{l:"پیشنهادات",a:()=>tab("suggestions")},{l:"مدیران",a:()=>tab("managers")},{l:"نشست‌ها",a:()=>tab("sessions")},{l:"نودها",a:()=>tab("nodes")},{l:"IPهای بسته",a:()=>tab("banned")},{l:"Workflows",a:()=>tab("workflows")},{l:"Cron",a:()=>tab("cron")},{l:"Webhooks",a:()=>tab("webhooks")},{l:"بحران",a:()=>tab("crisis")},{l:"مناطق IP",a:()=>tab("regions")},{l:"ISP",a:()=>tab("isp")},{l:"DNS",a:()=>tab("dns")},{l:"Upstreams",a:()=>tab("upstreams")},{l:"SpeedTest",a:()=>tab("speedtest")},{l:"Latency",a:()=>tab("latency")},{l:"DPI",a:()=>tab("dpi")},{l:"تنظیمات",a:()=>tab("settings")},{l:"پیشرفته",a:()=>tab("advanced")},{l:"API Keys",a:()=>tab("apikeys")},{l:"پشتیبان",a:()=>tab("backup")},{l:"لاگ‌ها",a:()=>tab("logs")},{l:"کاربر جدید",a:()=>ou()},{l:"خروج",a:()=>lo()}];' +
'function ckl(q){const c=$("ckl");c.innerHTML="";cmds.filter(x=>!q||x.l.includes(q)).forEach((x,i)=>{const d=document.createElement("div");d.className="ri"+(i===0?" on":"");d.textContent=x.l;d.onclick=()=>{x.a();$("cmdk").classList.remove("on")};c.appendChild(d)})}' +
...
'document.addEventListener("keydown",e=>{if((e.ctrlKey||e.metaKey)&&e.key==="k"){e.preventDefault();$("cmdk").classList.toggle("on");if($("cmdk").classList.contains("on")){$("ck2").value="";$("ck2").focus();ckl("")}}else if(e.key==="Escape"){["cmdk","um","gm","mm2","nm","rm","com","whm","bam","im","imc","wfm","upm"].forEach(id=>{if($(id)){$(id).style.display="none";$(id).classList.remove("on")}})}});' +
